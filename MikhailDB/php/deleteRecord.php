...
the code in catch-block is executed. */
    ...
    try {
        ...
        $sql = "SELECT ProjectID FROM $dbname.Project WHERE ProjectId = :ProjectID";
        $stmnt = $conn->prepare($sql);     // read about prered statements here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
        $stmnt->bindParam(':pdid', $_POST['prjId']);   // :stdId is the variable that we used in $sql, there must be a colon (:) in front of it.
        ...
        if ($stmnt->rowCount() === 0) {
            ...
            $sql = "DELETE FROM $dbname.Student WHERE ProjectID = :ProjectID";
